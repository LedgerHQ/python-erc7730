...
@pytest.mark.parametrize(
    "visible",
    [
        pytest.param({"ifNotIn": []}, id="ifNotIn"),
        pytest.param({"mustMatch": []}, id="mustMatch"),
    ],
)
def test_visibility_conditions_reject_empty_value_list(visible: dict[str, Any]) -> None:
    """
    The schema requires at least one value per condition ("minItems": 1).

    An empty list carries no condition at all, so it must fail here rather than reach calldata
    conversion, where it would be reported as an error on an already validated descriptor.
    """
    with pytest.raises(ValidationError):
        load_field({"path": "value", "label": "Value", "format": "raw", "visible": visible})
