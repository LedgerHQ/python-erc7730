...
def resolve_reference(
    ...
    if label is None and not _is_field_hidden(resolved_visible):
        return out.error(
...
def _is_field_hidden(resolved_visible: str | dict[str, Any] | None) -> bool:
    """Check whether resolved visibility rules make the field hidden (never displayed)."""
    if resolved_visible == "never":
        return True
    if isinstance(resolved_visible, dict) and "mustBe" in resolved_visible:
        return True
    return False
